...
void main() {
    vec2 pos = (v_vPosition - middle) * 2.0 / size;
	vec2 polar = cartesian_to_polar(pos);
	vec2 kal = make_kaleidoscope(polar, 4.0, time * SPEED);
	float noise = snoise(vec3(kal * SCALE, time));
	noise = smoothstep(0.0, 0.5, noise);
	vec4 col = mix(col_a, col_b, snoise(vec3(time, polar)));
	gl_FragColor = vec4(col.rgb, noise) * col.a;
}
